...
function getHtmlMsgItem(msg) {
    ...
    let $li = $("<li>").addClass("list-group-item list-group-item-action flex-column align-items-start").css("display", "grid");
...
async function fetchSentMessages() {
    ...
    return await fetch(msg_api + "/sent", options)
        .then((response) => {
...
async function fetchReceivedMessages() {
    ...
    return await fetch(msg_api + "/received", options)
        .then((response) => {
            return response.json();
        });
}
...
function loadReceivedMessages() {
    $('#received-msg-btn').addClass('active');
    fetchReceivedMessages().then((json) => {
