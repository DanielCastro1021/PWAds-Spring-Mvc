...
function loadImgList(img_arr) {
  for (let index in img_arr) {
    ...
    $img.attr(
      'style',
      'max-width: 100%; height: auto; object-fit: contain;margin-left: auto;margin-right: auto;'
    );
    $div.append($img);
...
async function fetchDeleteAd() {
  ...
  return await fetch(current_ad['_links']['self']['href'], options)
    .then((response) => {
      if (response.ok) {
        return response;
      }
