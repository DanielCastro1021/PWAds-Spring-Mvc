...
function sendMessage() {
    ...
    let adOwnerUsername = current_ad['owner']['username'];
...
function loadImgList(img_arr) {
    for (let index in img_arr) {
        ...
        if (img_arr.length > 1) $('#carousel-controls').hide();
        else $('#carousel-controls').show();
...
function loadAd(ad) {
    let $htmlAd = $('<li>').addClass("list-group-item list-group-item-action flex-column align-items-start").css("display", "grid");
    let $title = $('<h4>');
    let $description = $('<p>');
    let $owner = $('<small>');
    let $date = $('<small>');
    ...
    if (!ad.hasOwnProperty('imageList') || ad['imageList'] === null || ad['imageList'].length <= 0) loadNoImageFound();
    else loadImgList(ad['imageList']);
...
function updateBasicAd() {
    getBasicAdValues().then(fetchUpdateAd).then(r => {
        console.log(r)
...
function updatedCarAd() {
    getCarAdValues().then(fetchUpdateAd).then(r => {
        console.log(r)
        window.location.href = "../../html/fallout.html"
    });
}
...
function deleteAd() {
    fetchDeleteAd().then(fetchUpdateAd).then(r => {
        console.log(r)
        window.location.reload();
    });
}
